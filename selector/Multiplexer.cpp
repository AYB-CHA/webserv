...
void Multiplexer::run() {
    for (;;) {
        ...
        for (CIter it = write_clients.begin(); it != write_clients.end();
             ++it) {
            // std::cout << "I'm trying to write to: " << it->getSocketFd() <<
            // std::endl;
        ...
        for (CIter it = read_clients.begin(); it != read_clients.end(); ++it) {
            ...
            // Once it's set to true, the only reason it'd keep going is that we
            // still haven't started sending the response to the client
            // } else {
            //     // it->readChunk(); // This is where you just continue
            //     reading the body when necessary
            // }
